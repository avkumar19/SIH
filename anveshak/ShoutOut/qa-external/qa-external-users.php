...
function qa_get_login_links($relative_url_prefix, $redirect_back_to_url)
{
	...
	return array(
		'login' => 'http://localhost/SIH/anveshak/index.php/login',
		'register' => 'http://localhost/SIH/anveshak/index.php/registration',
		'logout' => 'http://localhost/SIH/anveshak/index.php/login/logout'
	);
...
function qa_get_logged_in_user()
{
	...
	if (isset($_SESSION['isLoggedIn'])) {
			$userid = $_SESSION['id'];
			if($_SESSION['type'] == 'startup'){
				$result = qa_db_read_one_assoc(qa_db_query_sub(
					"SELECT email FROM startup_contact WHERE startup_id = '$userid'"));
			}
			else{
				$result = qa_db_read_one_assoc(qa_db_query_sub(
					"SELECT email FROM corp_contact WHERE corp_id = '$userid'"));
			}	
			...
			if (is_array($result)) {
				return array(
					...
					'email' => $result['email'],
					'level' => ($userid == 'startup1234') ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC,
					'handle' => $userid
				);
			}
		}
...
function qa_get_user_email($userid)
{
	...
	qa_db_connection();
	$result = qa_db_read_one_assoc(qa_db_query_sub(
			'SELECT email FROM startup_contact WHERE startup_id=$,$userid UNION 
			SELECT email FROM corp_contact WHERE corp_id=$,$userid'
		));

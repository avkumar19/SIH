...
{
	var $table_corp = 'corp_auth';
	var $table_startup= 'startup_auth';
	var $table;
	...
	function validate_user($user_id, $pass,$type)
	{
		...
		$user_id = $this->authorization->strclean($user_id);
     	...
     	$row = $this->getUserById($user_id,$type);
		if($row !== false)
		{
			...
			if($password == $row->password )
			{
				...
				$data = $this->getUserById($user_id,$type);
    ...
    {
    	if($type=='startup')
    		$this->table=$this->table_startup;
    	else if($type=='corp')
    		$this->table=$this->table_corp;
    	else
    		return false;
